...
const descriptionsToggleBtn = document.querySelector('[data-action="toggle-descriptions"]')
const descriptionsStatusEl = document.getElementById('descriptions-status')
...
function renderDescriptionsToggle() {
  descriptionsToggleBtn.textContent = descriptionsEnabled ? 'Descriptions: ON' : 'Descriptions: OFF'
  descriptionsToggleBtn.classList.toggle('on', descriptionsEnabled)
  descriptionsStatusEl.textContent = descriptionsEnabled
    ? 'Step descriptions are burned onto the screen while running.'
    : ''
}

function render() {
  renderDescriptionsToggle()
  appEl.innerHTML = ''
  const byInstance = {}
...
async function loadDescriptions() {
  ...
  const json = await res.json()
  descriptionsEnabled = json.enabled
  renderDescriptionsToggle()
}

async function toggleDescriptions() {
  const res = await fetch('/api/descriptions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ enabled: !descriptionsEnabled })
  })
  const json = await res.json()
  descriptionsEnabled = json.enabled
  renderDescriptionsToggle()
